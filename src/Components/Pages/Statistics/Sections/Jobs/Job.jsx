...
const Job = ({ job }) => {
  ...
  return (
    <div className="border rounded-lg px-8 py-4">
      ...
      <h4 className="text-xl text-slate-500 font-semibold my-4">{company_name}</h4>
      <div className="flex gap-5 my-6">
        <button className={`btn btn-sm border-[#6940d8] hover:border-[#6940d8] font-bold rounded ${gradientText}`}>{remote_or_onsite}</button>
        <button className={`btn btn-sm border-[#6940d8] hover:border-[#6940d8]  font-bold rounded ${gradientText}`}>{job_type}</button>
      </div>
